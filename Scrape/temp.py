...
def hijack(obj, func_name_str, mode=""):
    ...
    def is_class(x):
        # Modify this function to use the correct `inspect` module for PaddlePaddle
...
with open(__file__.replace("__init__.py", "semi-processed\list_apis.txt"), "r") as f1:
    lines = f1.readlines()
    skipped = ["enable_grads", "get_default_dtype", "load", "tensor", "no_grad", "jit"]
    for l in lines:
        l = l.strip()
        if l not in skipped:
            module_obj = getattr(paddle, 'paddle')
            orig_func = getattr(module_obj, 'abs')
