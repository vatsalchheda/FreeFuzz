from classes.paddle_api import *
...
class PaddleLibrary(Library):
    ...
    def is_equal(x, y, diff_bound):
        def eq_float_tensor(x, y):
            # not strictly equal
            return paddle.allclose(x, y, atol=diff_bound, equal_nan=True)
...
def test():
    ...
    MyPaddle = PaddleLibrary("paddle-output")
    print(MyPaddle.generate_code(api, OracleType.CRASH))
    print(MyPaddle.generate_code(api, OracleType.CUDA))
    print(MyPaddle.generate_code(api, OracleType.PRECISION))
    ...
    # print(PaddleArgument.get_type(1))